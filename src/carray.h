...
public:
	...
	void RemoveAll(void) ;		// free stuff

	bool setLength(unsigned length) ;		// Set to new size
	int GetSize(void)					// Return cur length
		{ 
		return uEle; 
		} 
	...
	bool Add(TYPE data) ;					// Append data to end of array, growing array by 1.
 ...
 */
template <class TYPE, class T>
void CArray<TYPE,T>::RemoveAll(void)
{
	if (ptr != NULL)
		delete [] ptr ;
	ptr = NULL ;
 ...
 */
template <class TYPE, class T>
bool CArray<TYPE,T>::setLength(unsigned length)
{
	if (length == 0)
		{
		RemoveAll() ;
 ...
 */
template <class TYPE, class T>
bool CArray<TYPE,T>::set(TYPE data, unsigned index)
{
 ...
 */
template <class TYPE, class T>
bool CArray<TYPE,T>::get(TYPE &data, unsigned index)
{
	if (index >= uEle || ptr == NULL)
		return false ;
 ...
 */
template <class TYPE, class T>
bool CArray<TYPE,T>::get(TYPE *data) 
{
	if (data == NULL)
		return false ;
 ...
 */
template <class TYPE, class T>
bool CArray<TYPE,T>::remove(unsigned index) 
{
	if (uEle == 0 || ptr == NULL)
		return false ;
 ...
 */
template <class TYPE, class T>
bool CArray<TYPE,T>::insert(TYPE data, unsigned index) 
{
 ...
 */
template <class TYPE, class T>
bool CArray<TYPE,T>::Add(TYPE data) 
{
	// First setLength up one so we have room
	setLength( uEle+1 ) ;
